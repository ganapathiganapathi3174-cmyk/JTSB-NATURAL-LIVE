...
function extractAmount(ocrText, parsed) {
  ...
  const patterns = [
    ...
    /(?:AMOUNT|AMT|TOTAL|PAYMENT)\s*:?\s*(?:RS|INR|₹)?\s*([\d,]+\.?\d{0,2})/i,
    /([\d,]+\.?\d{0,2})\s*(?:CREDITED|DEBITED|TRANSFER|PAID|SENT)/i,
  ];
...
function extractUtr(ocrText, parsed) {
  ...
  const patterns = [
    /(?:UPI|RRN|REF|REFERENCE|TXN|TRXN)\s*(?:No|ID|REF|NUMBER)?\.?\s*:?\s*([A-Z0-9]{10,30})/i,
    /\b(\d{12,22})\b/,
  ];
  for (const pat of patterns) {
    const m = ocrText.match(pat);
...
function extractReceiverUpi(ocrText, parsed) {
  ...
  const patterns = [
    /(?:TO|RECEIVER|PAID\s*TO|TRANSFER\s*TO|BENEFICIARY)\s*:?\s*([\w.\-]+@[\w.]+)/i,
    /([\w.\-]+@(?:okicici|okaxis|okhdfc|oksbi|okbank|okyes|ibl|paytm|ptyes|ybl|razorpay|xpress))/i,
  ];
  for (const pat of patterns) {
    const m = ocrText.match(pat);
    if (m) {
...
function extractPaymentStatus(ocrText, parsed) {
  ...
  if (/\b(?:CREDITED|SUCCESS|SUCCESSFUL|COMPLETED|PAID|DONE|RECEIVED|SENT)\b/i.test(ocrText)) return { value: 'SUCCESS', source: 'regex', confidence: 'medium' };
  if (/\bDEBITED\b/i.test(ocrText)) return { value: 'DEBIT_SUCCESS', source: 'regex', confidence: 'medium' };
  if (/\b(?:PENDING|PROCESSING|INITIATED|AWAITING)\b/i.test(ocrText)) return { value: 'PENDING', source: 'regex', confidence: 'medium' };
  if (/\b(?:FAILED|REJECTED|DECLINED|CANCELLED|REVERSED|EXPIRED)\b/i.test(ocrText)) return { value: 'FAILED', source: 'regex', confidence: 'medium' };
  return { value: null, source: 'none', confidence: 'none' };
}
...
function extractReceiverName(ocrText, parsed) {
  ...
  const patterns = [
    /(?:BENEFICIARY|RECEIVER|TO)\s*:?\s*([A-Z][A-Za-z\s.]{2,40}?)(?:\s*(?:UPI|VIA|ON|AT|REF|\d))/i,
  ];
  for (const pat of patterns) {
    const m = ocrText.match(pat);
    if (m) {
...
function extractBankName(ocrText, parsed) {
  ...
  const banks = ['SBI', 'HDFC', 'ICICI', 'AXIS', 'KOTAK', 'YES BANK', 'PNB', 'CANARA', 'IDBI', 'INDUSIND', 'FEDERAL', 'RBL', 'IDFC'];
...
function run(ocrText, ocrWords) {
  ...
  log.info('', 'Extracted ' + fieldsFound + '/9 fields, parserConf=' + extracted.parserConfidence + '% (' + (Date.now() - t0) + 'ms)');
  return extracted;
}
