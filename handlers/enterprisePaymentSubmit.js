...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const errors = [];
    if (!paymentType || !['registration', 'topup'].includes(paymentType)) errors.push('Invalid paymentType (registration or topup)');
    ...
    if (!idForLookup) errors.push(paymentType === 'registration' ? 'pendingRegId is required for registration' : 'userId is required for topup');
    ...
    console.error('[enterprisePayment] Error:', e.message);
