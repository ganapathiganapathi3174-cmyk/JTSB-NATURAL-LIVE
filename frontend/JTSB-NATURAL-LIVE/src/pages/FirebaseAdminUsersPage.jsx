    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '550px' }}>
                ...
                {referrals.map((ref) => (
                  <div key={ref.id} style={{ padding: '0.5rem', background: '#1e293b', borderRadius: '4px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#e8eaef' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '0.75rem', marginTop: '0.25rem' }}>
                        {ref.referred_by_status === 'approved' ? (
                          ...
                          <span className="badge badge-rejected" style={{ fontSize: '0.7rem' }}>{ref.referred_by_status}</span>
                        )}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '0.35rem', alignItems: 'center' }}>
                      {(ref.referred_by_status === 'pending' || !ref.referred_by_status) && (
                        <button 
                          className={`btn btn-primary btn-sm${approving ? ' btn-loading' : ''}`}
        ...
        <div style={{ marginTop: '1.5rem', borderTop: '1px solid #ddd', paddingTop: '1rem' }}>
          <button 
            className={`btn btn-primary${resetting ? ' btn-loading' : ''}`}
                          ...
                          <>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                              {expandedReferrals.map((ref) => (
                                <div key={ref.id} style={{ padding: '0.5rem', background: '#1e293b', borderRadius: '4px', border: '1px solid var(--border)', color: '#e8eaef', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  <div>
                                    <div style={{ fontWeight: 'bold', color: '#e8eaef' }}>{ref.name}</div>
                                    ...
                                    <div style={{ fontSize: '0.75rem', marginTop: '0.25rem' }}>
                                      {ref.referred_by_status === 'approved' ? (
                                        ...
                                        <span className="badge badge-pending" style={{ fontSize: '0.7rem' }}>Pending Approval</span>
                                      ) : (
                                        <span className="badge badge-rejected" style={{ fontSize: '0.7rem' }}>{ref.referred_by_status}</span>
                                      )}
                                    </div>
